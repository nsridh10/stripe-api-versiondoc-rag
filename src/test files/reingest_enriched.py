...
hypothetical questions) and re-ingests it into ChromaDB with the enrichment
data fused directly into the page content for improved semantic search.
...
def reingest_enriched_data():
    """
    Re-ingests enriched documentation data into ChromaDB.
    
    Reads from data/enriched_db.json, fuses the enrichment data into the
    content, clears the old collection, and re-adds all documents.
    """
    ...
    for idx, item in enumerate(enriched_data):
        try:
            ...
            if q_str and not q_str.startswith("-"):
                q_str = "- " + q_str
