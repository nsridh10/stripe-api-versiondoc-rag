...
> Source: [`src/ingestion.py`](src/ingestion.py), [`src/parsers.py`](src/parsers.py), [`config.yaml`](config.yaml)
...
> Source: [`src/graph/`](src/graph/), [`src/prompts/`](src/prompts/), [`src/tools/search.py`](src/tools/search.py)

The core of the system is an **8-node LangGraph state machine** defined in [`src/graph/builder.py`](src/graph/builder.py). Each request flows through the graph with conditional routing — nodes can route to each other, skip to `END`, or loop for retries.
...
All nodes read from and write to a shared `AgentState` (defined in [`src/graph/state.py`](src/graph/state.py)):
...
Conditional routing is defined in [`src/graph/routing.py`](src/graph/routing.py):
...
> Source: [`src/memory.py`](src/memory.py), [`src/routes/query.py`](src/routes/query.py)
...
> Source: [`src/eval/eval_llm.py`](src/eval/eval_llm.py), [`src/eval/test_cases.py`](src/eval/test_cases.py), [`src/eval/README.md`](src/eval/README.md)
